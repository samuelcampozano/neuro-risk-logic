...
class RealDataPreprocessor:
    ...
    def clean_binary_column(self, series: pd.Series) -> pd.Series:
        ...
        positive_values = [
            'yes', 'si', 'sí', 'y', 's', '1', 'true', 'verdadero',
            'present', 'presente', 'positive', 'positivo', 'x'
        ]
    ...
    def clean_gender_column(self, series: pd.Series) -> pd.Series:
        ...
        gender_map = {
            ...
            'o': 'Other', 'otro': 'Other', 'other': 'Other'
        }
    ...
    def clean_social_support(self, series: pd.Series) -> pd.Series:
        ...
        # If text, try to map
        support_map = {
            'bajo': 'isolated', 'low': 'isolated', 'aislado': 'isolated',
            'medio': 'moderate', 'moderate': 'moderate', 'moderado': 'moderate',
            'alto': 'supported', 'high': 'supported', 'apoyado': 'supported'
        }
    ...
    def preprocess_data(self) -> pd.DataFrame:
        ...
        for feature, source_col in analysis['found_mappings'].items():
            ...
            if feature == 'age':
                ...
                processed_data['social_support_level'] = self.clean_social_support(self.df[source_col])
        ...
        for feature_def in feature_defs['features']:
            ...
            if feature_name in self.processed_df.columns:
                if feature_name in self.binary_features:
                    ...
                    risk_scores += self.processed_df[feature_name].map(support_scores).fillna(weight * 0.5)
    ...
    def save_processed_data(self, output_path: Optional[str] = None):
        ...
        report = {
            ...
            "missing_data_summary": {
                col: self.processed_df[col].isna().sum() 
    ...
    def generate_quality_report(self) -> Dict[str, Any]:
        ...
        for col in self.processed_df.columns:
            ...
            feature_report = {
                ...
                "missing_pct": f"{(self.processed_df[col].isna().sum() / len(self.processed_df)) * 100:.1f}%"
            }
        ...
        report["correlations"] = dict(sorted(
            risk_correlations.items(), 
        ...
        low_variance_features = [
            col for col in self.binary_features 
            if col in self.processed_df.columns and 
            (self.processed_df[col].mean() < 0.05 or self.processed_df[col].mean() > 0.95)
        ]
        
        if low_variance_features:
            report["recommendations"].append(
                f"Low variance features: {', '.join(low_variance_features)}. May not be informative."
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess real clinical data for NeuroRiskLogic"
    )
    parser.add_argument(
        "input_file",
        type=str,
