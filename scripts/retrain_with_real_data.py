...
class IncrementalModelTrainer:
    ...
    def __init__(self, min_new_samples: int = 50):
        ...
        try:
            ...
            if predictor.is_loaded:
                ...
                logger.info(f"Loaded current model version: {self.model_metadata.get('version', 'unknown')}")
    ...
    def collect_training_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if synthetic_files:
            ...
            logger.info(f"Loaded {len(synthetic_df)} synthetic samples from {latest_synthetic.name}")
    ...
    def train_incremental_model(
        ...
        if self.current_model is not None:
            ...
            current_auc = roc_auc_score(y_val, current_pred_proba) if len(np.unique(y_val)) > 1 else 0.0
    ...
    def save_model_version(self, model: Any, metrics: Dict[str, Any], data_composition: Dict[str, int]):
        """
        ...
        if current_model_path.exists():
            backup_path = settings.models_dir / f"model_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
    ...
    def should_retrain(self, database_df: Optional[pd.DataFrame]) -> bool:
        ...
        if len(database_df) < self.min_new_samples:
            logger.info(f"Only {len(database_df)} new samples, need {self.min_new_samples} for retraining")
            return False
    ...
    def run_retraining_pipeline(self, force: bool = False) -> Dict[str, Any]:
        ...
        if 'improvement' in metrics:
            if metrics['improvement'] > 0:
                ...
                logger.warning(f"  ⚠️ Model performance decreased by {abs(metrics['improvement']):.4f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Retrain model with real clinical data"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--real-data",
        type=str,
        help="Path to new real clinical data file"
    )
