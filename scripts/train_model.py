    ...
    def load_data_from_database(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            if len(assessments) < 50:
                raise ValueError(
                    f"Insufficient data: {len(assessments)} samples (minimum 50 required)"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train neurodevelopmental risk assessment model")
    parser.add_argument(
        "-s",
        "--source",
    ...
    parser.add_argument("-f", "--file", type=str, help="CSV file path (if source is csv)")
    parser.add_argument("-v", "--version", type=str, default=None, help="Model version string")
    ...
    if args.source == "csv":
        if not args.file:
            ...
            if not csv_files:
                raise ValueError(
                    "No synthetic data files found. Run generate_synthetic_data.py first."
                )
