...
def corregir_pregunta(pregunta):
    ...
    correcciones = {
        ...
        "Does the subject have a family history of neurological disorders?": "¿Tengo antecedentes familiares de trastornos neurológicos o psiquiátricos?",
        "Has the subject ever had seizures or convulsions?": "¿He tenido alguna vez convulsiones o crisis epilépticas?",
        "History of traumatic brain injury (TBI) or head trauma?": "¿He tenido alguna lesión cerebral traumática o trauma craneal?",
        "Diagnosed with any psychiatric disorder?": "¿He sido diagnosticado/a con algún trastorno psiquiátrico?",
        "Does the subject consume psychoactive substances regularly?": "¿Consumo sustancias psicoactivas regularmente?",
        "History of suicide attempts or ideation?": "¿He tenido intentos de suicidio o ideación suicida?",
        "Is the subject on psychotropic or neurological medication?": "¿Estoy tomando medicación psicotrópica o neurológica?",
        "Was the subject born with complications or low weight?": "¿Nací con complicaciones o bajo peso?",
        "Does the subject live in extreme poverty?": "¿Vivo en condiciones de pobreza extrema?",
        "Has the subject ever had access restrictions to education?": "¿He tenido restricciones de acceso a la educación?",
        "Does the subject have access to mental healthcare?": "¿Tengo acceso a servicios de salud mental?",
        "Has the subject ever been diagnosed with a disability?": "¿He sido diagnosticado/a con alguna discapacidad?",
        "Was the subject breastfed during infancy?": "¿Fui amamantado/a durante la infancia?",
        "Exposure to violence or trauma (childhood or adulthood)?": "¿He estado expuesto/a a violencia o trauma (infancia o adultez)?"
    }
...
def extraer_toda_informacion():
    ...
    for i, p in enumerate(preguntas_info, 1):
        ...
        print(f"   - Tipo: {'Factor de Riesgo' if p['direccion_riesgo'] == 'positive' else 'Factor Protector'}")
    ...
    for nivel, datos in escenarios.items():
        ...
        risk_factors = []
        if datos.get('family_neuro_history'): risk_factors.append("Family history")
        if datos.get('psychiatric_diagnosis'): risk_factors.append("Psychiatric diagnosis")
        if datos.get('suicide_ideation'): risk_factors.append("Suicide ideation")
        
        recomendaciones = calculator.generate_recommendations(
            nivel.lower().replace(" ", "_"),
            risk_factors,
            datos_completos
        )
