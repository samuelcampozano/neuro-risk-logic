...
# Configure logging
logger.add("logs/model_evaluation.log", rotation="10 MB", level="INFO")
...
class ModelEvaluator:
    ...
    def create_evaluation_plots(
        self, X_test: np.ndarray, y_test: np.ndarray, output_dir: Path = None
    ):
        """
        ...
        plt.figure(figsize=(8, 6))
        plt.plot(
            recall, precision, color="darkgreen", lw=2, label=f"PR curve (AP = {avg_precision:.3f})"
        )
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel("Recall")
        ...
        plt.subplot(1, 2, 2)
        plt.boxplot(
            [y_pred_proba[y_test == 0], y_pred_proba[y_test == 1]], labels=["Low Risk", "High Risk"]
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate neurodevelopmental risk model")
    parser.add_argument("--model", type=str, help="Path to model file (default: current model)")
    ...
    parser.add_argument("--compare", action="store_true", help="Compare all model versions")
