...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> TokenData:
    ...
    try:
        ...
        token_data = TokenData(
            username=username, scopes=scopes, exp=datetime.fromtimestamp(exp) if exp else None
        )
...
def get_admin_token(expires_delta: Optional[timedelta] = None) -> Token:
    ...
    access_token = create_access_token(
        data={"sub": "admin", "scopes": ["admin", "read", "write", "retrain"]},
...
async def require_write_permission(token_data: TokenData = Depends(verify_token)) -> TokenData:
    """
...
async def require_retrain_permission(token_data: TokenData = Depends(verify_token)) -> TokenData:
    ...
    if "retrain" not in token_data.scopes and "admin" not in token_data.scopes:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Model retrain permission required"
        )
    return token_data
...
async def get_api_key_header(x_api_key: str = Header(..., alias="X-API-Key")) -> str:
    """
