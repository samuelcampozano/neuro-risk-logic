...
class TrainingRequest(BaseModel):
    ...
    n_samples: int = Field(
        default=1000, ge=100, description="Number of synthetic samples to generate"
    )
    test_size: float = Field(
        default=0.2, ge=0.1, le=0.5, description="Proportion of data for testing"
    )
    create_visualizations: bool = Field(default=True, description="Generate visualization plots")
...
def run_training_task(task_id: str, request: TrainingRequest):
    ...
    try:
        ...
        generator = SyntheticDataGenerator(
            n_samples=request.n_samples, random_seed=settings.random_seed
        )
...
async def start_training(
    ...
    active_tasks = [tid for tid, task in training_tasks.items() if task.get("status") == "running"]
    ...
    return TrainingResponse(
        status="started", message="Training task started successfully", task_id=task_id
    )
...
@router.get("/history", summary="Get training history", description="Get history of training tasks")
async def get_training_history(
    ...
    for task_id, task_info in sorted_tasks[:limit]:
        ...
        if task_info.get("status") == "completed":
            ...
            if "metrics" in task_info:
                summary["metrics"] = {
                    k: v for k, v in task_info["metrics"].items() if isinstance(v, (int, float))
                }
    ...
    return {"total_tasks": len(training_tasks), "history": history}
...
async def clear_training_history(token_data: TokenData = Depends(require_admin)) -> Dict[str, str]:
    """
    ...
    tasks_to_remove = [
        tid for tid, task in training_tasks.items() if task.get("status") in ["completed", "failed"]
    ]
    ...
    return {"message": f"Cleared {len(tasks_to_remove)} tasks from history"}
