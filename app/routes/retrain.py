...
class RetrainingRequest(BaseModel):
    ...
    force: bool = Field(
        default=False,
        description="Force retraining even if conditions aren't met"
    )
    ...
    include_synthetic: bool = Field(
        default=True,
        description="Include synthetic data in training"
    )
...
async def start_retraining(
    ...
    active_tasks = [
        tid for tid, task in retraining_tasks.items()
...
@router.get(
    ...
    description="Get history of retraining tasks"
)
async def get_retraining_history(
    ...
    for task_id, task_info in sorted_tasks[:limit]:
        ...
        if "result" in task_info:
            ...
            if "metrics" in task_info["result"]:
                summary["metrics"] = {
                    k: v for k, v in task_info["result"]["metrics"].items()
    ...
    return {
        "total_tasks": len(retraining_tasks),
        "history": history
    }
...
async def compare_model_versions(
    ...
    try:
        # Find model files
        model_files = sorted(
            settings.models_dir.glob("model_v*.pkl"),
