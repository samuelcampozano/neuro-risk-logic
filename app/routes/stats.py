...
async def get_statistics(
    ...
    try:
        ...
        assessments_by_risk_level = {
            level: count for level, count in risk_level_counts
        }
        ...
        gender_counts = db.query(
            Assessment.gender,
            func.count(Assessment.id)
        ).group_by(Assessment.gender).all()
        
        assessments_by_gender = {
            gender: count for gender, count in gender_counts
        }
        ...
        for assessment in high_risk_assessments:
            ...
            if assessment.consanguinity:
                risk_factor_frequency["consanguinity"] = risk_factor_frequency.get("consanguinity", 0) + 1
            if assessment.family_neuro_history:
                risk_factor_frequency["family_neuro_history"] = risk_factor_frequency.get("family_neuro_history", 0) + 1
            if assessment.seizures_history:
                risk_factor_frequency["seizures_history"] = risk_factor_frequency.get("seizures_history", 0) + 1
            if assessment.brain_injury_history:
                risk_factor_frequency["brain_injury_history"] = risk_factor_frequency.get("brain_injury_history", 0) + 1
            if assessment.psychiatric_diagnosis:
                risk_factor_frequency["psychiatric_diagnosis"] = risk_factor_frequency.get("psychiatric_diagnosis", 0) + 1
            if assessment.substance_use:
                risk_factor_frequency["substance_use"] = risk_factor_frequency.get("substance_use", 0) + 1
            if assessment.suicide_ideation:
                risk_factor_frequency["suicide_ideation"] = risk_factor_frequency.get("suicide_ideation", 0) + 1
            if assessment.extreme_poverty:
                risk_factor_frequency["extreme_poverty"] = risk_factor_frequency.get("extreme_poverty", 0) + 1
            if assessment.violence_exposure:
                risk_factor_frequency["violence_exposure"] = risk_factor_frequency.get("violence_exposure", 0) + 1
        ...
        if total_high_risk > 0:
            for factor, count in sorted(
                risk_factor_frequency.items(), 
                key=lambda x: x[1], 
                reverse=True
            )[:5]:
                frequency = count / total_high_risk
                most_common_risk_factors.append({
                    "factor": factor,
                    "frequency": frequency
                })
        ...
        recent_count = db.query(Assessment).filter(
...
async def get_trends(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error calculating trends"
        )
...
async def analyze_risk_factors(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error analyzing risk factors"
        )