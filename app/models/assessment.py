...
class Assessment(Base):
    ...
    family_neuro_history = Column(
        Boolean, nullable=False, comment="Family history of neurological disorders"
    )
    seizures_history = Column(Boolean, nullable=False, comment="History of seizures or convulsions")
    brain_injury_history = Column(
        Boolean, nullable=False, comment="History of traumatic brain injury"
    )
    psychiatric_diagnosis = Column(
        Boolean, nullable=False, comment="Diagnosed with psychiatric disorder"
    )
    substance_use = Column(Boolean, nullable=False, comment="Regular psychoactive substance use")
    suicide_ideation = Column(
        Boolean, nullable=False, comment="History of suicide attempts or ideation"
    )
    psychotropic_medication = Column(
        Boolean, nullable=False, comment="Currently on psychotropic medication"
    )

    # Sociodemographic Features (Binary)
    birth_complications = Column(
        Boolean, nullable=False, comment="Born with complications or low weight"
    )
    extreme_poverty = Column(Boolean, nullable=False, comment="Lives in extreme poverty")
    education_access_issues = Column(
        Boolean, nullable=False, comment="Restricted access to education"
    )
    ...
    social_support_level = Column(
        String(20), nullable=False, comment="Level of social support (isolated/moderate/supported)"
    )

    # Risk Assessment Results
    risk_score = Column(Float, nullable=False, comment="Calculated risk score (0.0 to 1.0)")
    risk_level = Column(String(20), nullable=False, comment="Risk category (low/moderate/high)")
    confidence_score = Column(
        Float, nullable=True, comment="Model confidence in prediction (0.0 to 1.0)"
    )

    # Feature importance snapshot
    feature_contributions = Column(
        JSON, nullable=True, comment="Individual feature contributions to risk score"
    )

    # Metadata
    model_version = Column(String(50), nullable=False, comment="Version of ML model used")
    assessment_date = Column(
        DateTime, default=datetime.utcnow, nullable=False, comment="Timestamp of assessment"
    )

    # Optional fields for tracking
    clinician_id = Column(
        String(100), nullable=True, comment="ID of clinician who requested assessment"
    )
    notes = Column(Text, nullable=True, comment="Additional clinical notes")

    # Consent and compliance
    consent_given = Column(
        Boolean, default=False, nullable=False, comment="Subject consent for data use"
    )
    ...
    def _encode_social_support(self) -> int:
        ...
        mapping = {"isolated": 0, "moderate": 1, "supported": 2}
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "id": self.id,
            "demographics": {"age": self.age, "gender": self.gender},
            "clinical_factors": {
