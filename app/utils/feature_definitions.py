...
class FeatureDefinitions:
    ...
    def get_high_risk_features(self) -> List[str]:
        ...
        return [
            name for name, info in self.features.items() if info.get("risk_direction") == "positive"
        ]

    def get_protective_features(self) -> List[str]:
        """Get list of features with negative risk direction."""
        return [
            name for name, info in self.features.items() if info.get("risk_direction") == "negative"
        ]

    def calculate_feature_contributions(
        self, feature_values: Dict[str, Any], feature_importances: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
...
def get_feature_importance(
    feature_values: Dict[str, Any], model_importances: Optional[Dict[str, float]] = None
) -> Dict[str, float]:
    """
    ...
    """
    definitions = load_feature_definitions()
    return definitions.calculate_feature_contributions(feature_values, model_importances)
